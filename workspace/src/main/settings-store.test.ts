...
describe("SettingsStore", () => {
  it("기본값으로 시작하고 update로 반영·저장된다", async () => {
    ...
    const updated = await store.update({ logLevel: "debug", fileSizeLimitBytes: 4 * 1024 * 1024 });
    expect(updated).toMatchObject({ logLevel: "debug", fileSizeLimitBytes: 4 * 1024 * 1024 });
    expect(store.current).toMatchObject(updated);

    const reloaded = new SettingsStore(path.join(directory, "settings.json"));
    await expect(reloaded.load()).resolves.toMatchObject({ logLevel: "debug" });
  });

  it("잘못된 값은 무시하고 기존 값을 유지한다", async () => {
    const directory = await mkdtemp(path.join(os.tmpdir(), "workspace-settings-invalid-"));
    const store = new SettingsStore(path.join(directory, "settings.json"));
    const updated = await store.update({ logLevel: "not-a-level" as never, fileSizeLimitBytes: 10 });
  ...
  it("파일이 없으면 기본값을 반환한다", async () => {
    ...
    await expect(store.load()).resolves.toMatchObject({ logLevel: "info" });
  });
});
