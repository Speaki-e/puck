...
//   4. `.env` in `~/Library/Application Support/Shaydi/` -- the same folder
...
struct AgentConfiguration {
    ...
    /// apps can reach. Shaydi shows the field, ShaydiAgent runs the
    ...
    static var supportDirectory: URL {
        URL(fileURLWithPath: NSHomeDirectory())
            .appendingPathComponent("Library/Application Support/Shaydi", isDirectory: true)
    }
    ...
    static var repositoryDirectory: URL? {
        #if DEBUG
        // …/pet-app/Shaydi/Agent/AgentConfiguration.swift -> …/pet-app
        return URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // Agent
            .deletingLastPathComponent() // Shaydi
