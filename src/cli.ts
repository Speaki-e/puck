/**
 * ai-module A3 — CLI 테스트 진입점.
 *
 *   npm run cli "명령"          단발 실행 (텍스트 스트리밍 + 도구 로그)
 *   npm run cli                 대화형 REPL
...
async function main(): Promise<void> {
  ...
  if (mode === "seq" && command === "") {
    ...
    console.error('사용법: npm run cli --seq "명령"');
    process.exitCode = 1;
    return;
  }
  ...
  switch (mode) {
    case "seq":
      process.exitCode = await runSeq(client, command);
      return;
    case "repl":
 ...
 * 삼켜 스크립트까지 전달하지 않고 npm_config_seq=true만 남긴다. 그래서 두
 * 경로를 모두 본다 — 직접 실행(`npx tsx src/cli.ts --seq "명령"`)과
 * npm 경유 실행이 같게 동작해야 한다.
 */
function parseArgs(argv: string[]): { mode: Mode; command: string } {
  const seq = argv.includes("--seq") || process.env["npm_config_seq"] === "true";
  const command = argv.filter((arg) => arg !== "--seq").join(" ").trim();

  if (seq) return { mode: "seq", command };
  // 명령 없이 실행하면 REPL. (A2까지는 기본 명령을 대신 실행했다.)
  return { mode: command === "" ? "repl" : "once", command };
}
