...
//  PetAgentClient's main menu, built in code.
//
...
//  the smallest menu that makes them work again. PetAgentTests compiles this
//  one file directly (see project.yml) rather than reaching it through the
//  PetAgent target, which never uses it.
//
...
enum ClientMainMenu {
    static func make(appName: String = "PetAgentClient") -> NSMenu {
        let mainMenu = NSMenu()
    ...
    private static func appMenu(appName: String) -> NSMenu {
        ...
        menu.addItem(.separator())
        // Quits this app only. The pet is a separate process (PetAgent) and
