...
> **2026-08-12 v6 통합 기준:** 아래 기존 체크리스트의 초기 진단은 역사 기록으로 남겨둔다. 실제 코드 기준 A0~A7 핵심 경로(세션/컨텍스트/승인/`open_task_session`/Workspace 실행기 연결)는 구현됐다. 현재 우선순위는 실제 pet-app+Claude E2E, executor timeout, protocol `src: agent` 로그, A8 회귀 케이스 확장이다.
...
- v6 기준 실제 구현 코드는 `src/`에 있으며 A0~A6 핵심 경로가 동작하도록 구성돼 있다.
- 2026-08-12 통합본에서 공개 entrypoint(`src/index.ts`), protocol 패키지 루트 import, 이미지 attachment 전달, 번들 환경용 안전 프롬프트/화이트리스트 폴백을 보강했다.
- Workspace 통합은 `packages/ai-module` + `AiModuleRuntime`으로 연결했다. Workspace별 Claude client를 재사용해 sessionId 히스토리를 유지하며 세션별 editorLocal은 동시 실행에서도 격리된다.
- 아직 실제 pet-app/Claude 인증 환경 전체 E2E, executor timeout, protocol 형식 agent 로그, A8 회귀 케이스 확장은 남아 있다.
