...
**Last updated:** 2026-07-29 · **Tests:** 664 passing (`xcodebuild test`) · **`main`:** `0efb4c2`

**2026-07-29: F13 (Claude-Desktop-style client window) foundation landed — protocol schema, pet-app's Bridge mirror, and the Pinned FSM state. The window itself (sidebar/chat/embedded editor) is not yet built.** byeolki asked for a redesign toward multiple chat sessions, a workspace switcher, and an embedded editor view (see `plan/*.md`'s 2026-07-29 entries for the full design conversation), then said to start building it in protocol and pet-app. Landed so far:
- **protocol (`0.3.0` + 2 follow-up commits)**: `user_input` gains `workspace_id`/`session_id`/`attachments` (all optional, default `"default"`/absent); new `workspace_create(_request)`/`session_create(_request)` messages; `editor_view_ready`/`editor_view_unavailable` (workspace serves the embeddable editor bundle over its own local loopback server, never over `bridge.sock`); `approval_response`/`run_cancel` (the allow/deny and stop-button UI moved from workspace's renderer into pet-app, so resolving them now round-trips the socket); `open_task_session` — the one tool that never dispatches, letting the agent branch a casual conversation into a task session on its own judgement. Caught and fixed a real gap while wiring pet-app's receiving side: `workspace_id`/`session_id` need to be on *every* event, not just `await_approval`, or pet-app can't tell which session's timeline an incoming `agent_thinking`/`tool_call`/`tool_result`/`agent_done` belongs to once more than one session is open.
- **pet-app**: `BridgeMessages.swift` mirrors all of the above (Codable round-trip tests for every new message). `BridgeMessageRouter` routes `workspace_create`/`session_create`/`editor_view_ready`/`editor_view_unavailable` through a new `onClientUpdate` callback (intended consumer: the not-yet-built F13 data store), and the four pet-app-only-sends message kinds are now correctly no-ops. New `PinnedState` (trivial, same "capture current state, restore on close" pattern as `Listen`) — `Option+Shift+Space` now pins the character for the input bubble's duration; when the real client window replaces that bubble, the pin/unpin calls carry over to its show/close hooks.
- **Known limitation, not yet addressed**: the chat view's "streaming text + tool-call timeline" (F13) needs richer per-event data (raw assistant text, tool_use ids/args/data) than the current coarse `BridgeEvent` vocabulary carries — that's workspace/ai-module's side to eventually emit, out of scope for a protocol+pet-app-only pass. A v1 chat view can render from today's coarse events (thinking/tool name/success-fail/approval/done) without it.
- **Still to build**: the actual F13 window (sidebar, workspace switcher, session list, chat rendering, embedded `WKWebView` editor view) and F14 (drag capture, marked optional/droppable in `plan/02_pet-app.md`'s own risk table).
