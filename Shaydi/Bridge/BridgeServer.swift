...
//  Network.framework NWListener(UDS) at ~/Library/Application Support/Shaydi/bridge.sock
//
...
final class BridgeServer {
    ...
    private let queue = DispatchQueue(label: "Shaydi.BridgeServer")
    ...
    /// pin/unpin the character: ShaydiAgent is a Dock-resident app
    /// expected to stay running continuously (byeolki: "둘이 같이
    /// 가야하는거임"), so tying the pin to "is a gui connected" would freeze
    /// the pet in place for as long as ShaydiAgent is alive, not just
    ...
    /// ShaydiAgent's chat view (2026-07-30) -- `broadcast` deliberately
    ...
    /// principle, even if only one ShaydiAgent is expected in practice).
...
extension BridgeServer: UserInputTransport {
    ...
    ///
    /// gui-role connections don't count -- ShaydiAgent is a chat *view*,
